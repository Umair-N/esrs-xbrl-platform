...
from models import ReportBlock, ReportDocument, TextUpload
...
def save_report_to_db(report: ReportDocument, user_id: int, db) -> bool:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error saving report to database: {str(e)}"
        )
...
async def get_report(
    report_id: str, current_user: dict = Depends(get_current_user), db=Depends(get_db)
):
